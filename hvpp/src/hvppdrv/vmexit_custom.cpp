...
void vmexit_custom_handler::handle_execute_vmcall(vcpu_t& vp) noexcept
{
  ...
  switch (vp.context().ecx)
  {
  case 0xc1: {
    {
      cr3_guard _{ vp.guest_cr3() };

      data.page_map.push_back({
        pa_t::from_va((void*)vp.context().edx),
        pa_t::from_va((void*)vp.context().ebx) });
    }

    hvpp_trace("vmcall (hook) EXEC (VA): 0x%p READ (VA): 0x%p",
      ia32::detail::va_from_pa(data.page_map.front().page_exec.value()),
      ia32::detail::va_from_pa(data.page_map.front().page_exec.value()));

    //
    // Split the 2MB page where the code we want to hook resides.
    //
    vp.ept().split_2mb_to_4kb(data.page_map.front().page_exec & ept_pd_t::mask, data.page_map.front().page_exec & ept_pd_t::mask);

    //
    // Set execute-only access on the page we want to hook.
    //
    vp.ept().map_4kb(data.page_map.front().page_exec, data.page_map.front().page_exec, epte_t::access_type::execute);
  ...
  case 0xc2:
    ...
    //
    for (auto page : data.page_map) {
      vp.ept().join_4kb_to_2mb(page.page_exec & ept_pd_t::mask, page.page_exec & ept_pd_t::mask);
    }
    data.page_map.clear();
...
void vmexit_custom_handler::handle_ept_violation(vcpu_t& vp) noexcept
{
  ...
  if (exit_qualification.data_read || exit_qualification.data_write)
  {
    ...
    for (auto page : data.page_map) {
      vp.ept().map_4kb(page.page_exec, page.page_read, epte_t::access_type::read_write);
    }
  }
  else if (exit_qualification.data_execute)
  {
    ...
    for (auto page : data.page_map) {
      vp.ept().map_4kb(page.page_exec, page.page_exec, epte_t::access_type::execute);
    }
  }
